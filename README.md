...
### Autoiter 循环 (`/collie-harness:autoiter`)

**定位**：长时运行 × 指标驱动 × 强防过拟合的自迭代修复闭环。

**用法**：
```
/collie-harness:autoiter "<task>" [--max-iterations N] [--budget-tokens M] [--mode interactive|queued]
```
...
任何 plan 若改动用户可见行为 / 架构约束 / 已有文档内容，必须包含显式的文档更新任务（README / CLAUDE.md / docs/\*-spec.md / `.claude/skills/*/SKILL.md`（若改动涉及项目级 SOP/操作清单））。由 `collie-harness:plan-doc-reviewer` 的 Doc Maintenance 检查、`collie-harness:review` Red line #12，以及 `gated-workflow` Step 5.5 共同强制。
...
| `memory/bump-access.js` | PostToolUse(Read) | 被读取的 `~/.memory-palace/` 文件自动 bump access_count |
...
│   ├── autoiter.md                       # /collie-harness:autoiter slash command
