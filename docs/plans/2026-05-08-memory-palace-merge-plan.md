...
用户决定将其合并到 collie plugin 中，而非作为独立 plugin 维护。原因：当前只 for Claude Code，避免维护两个 plugin 的开销。未来如需多 agent 适配再拆出。

## 源码位置

- memory-palace 源：`~/git/memory-palace/`
- collie 源：`~/git/collie/`
- collie 安装位置：`~/.claude/plugins/cache/collie-marketplace/collie/0.3.0/`
...
collie 版本从 0.3.0 → 0.4.0（新增 memory-palace 功能模块）。
...
1. collie 中创建 worktree
2. 复制文件（cp，不用 Write）
3. 调整脚本路径
4. 合并 hooks.json
5. 跑测试
6. 清理旧 symlink
7. skillhub upsert 发布 collie 0.4.0
