...
5. **writing-plans Plan Review Loop 与 collie 双审重复**: `writing-plans` 内置 `plan-document-reviewer` per-chunk 审查，collie 在 auto.md Step ③ 另有 `plan-doc-reviewer` + `collie:review` 双审。两轮审查用不同 rubric，但 agent 可能困惑。
...
**外部方案**: 无可复用的开源方案。这是 collie 自身 prompt 工程 + hook 逻辑的修正，纯内部改动。
...
collie 工作流在"规划→执行"衔接处有 3 个核心缺陷 + 3 个 superpowers 兼容性问题 + 1 个上下文浪费：
...
| Fix | 改哪里 | 改什么 |
|-----|--------|--------|
| F1a | `commands/auto.md` | 新增约束：plan 第三行 metadata `<!-- plan-executor: collie:gated-workflow -->` |
| F1b | `commands/auto.md` | 新增约束：prose header 覆写为 `MUST invoke Skill('collie:gated-workflow')` |
...
```
writing-plans 内置的 Plan Review Loop（plan-document-reviewer per-chunk 审查）在 collie 中跳过。
collie 在 Step ③ 有更严格的双 reviewer 审查（plan-doc-reviewer + collie:review），不重复运行。
```

**F5 — 跳过 writing-plans Execution Handoff**：
```
writing-plans 的 Execution Handoff（"Ready to execute?" + skill 推荐）在 collie 中跳过。
collie 通过 auto.md → gated-workflow 控制执行流程。
...
3. E2E 验收：用 `/collie:auto` 跑一个小任务，检查生成的 plan 文件包含三条 metadata + 正确的 prose header
...
**Goal:** 修复 collie 工作流在"规划→执行"衔接处的 7 个缺陷，确保 /clear 后 agent 使用正确的执行 skill、plan 包含 task DAG、CR fix 不污染主 session context。
...
- plan header "For agentic workers" 行指向 collie:gated-workflow（正面指令，不加负面语句）
...
- 在 `<!-- plan-topic: ... -->` 后加 `<!-- plan-executor: collie:gated-workflow -->`
...
```
>     - **Skip writing-plans Plan Review Loop**: writing-plans 内置的 plan-document-reviewer per-chunk 审查在 collie 中跳过。collie 在 Step ③ 有更严格的双 reviewer 审查，不重复运行。
>     - **Skip writing-plans Execution Handoff**: writing-plans 的 "Ready to execute?" + skill 推荐在 collie 中跳过。Plan 写完后直接回到 auto.md Step ③ 的双 review。
...
- 在 metadata 示例 block 中加第三行 `<!-- plan-executor: collie:gated-workflow -->`
...
- "ExitPlanMode with reviewed plan → stdout empty" test (line 119-140)：plan content 加入 `<!-- plan-executor: collie:gated-workflow -->\n`
