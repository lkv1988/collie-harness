...
对照 Google eng-practices 的 reviewer checklist（"Does this break callers?"、"Are tests adequate for the change surface?"），成熟工程实践要求 plan 阶段显式枚举两个环：**直接影响**（被修改的模块 / 文件 / API）与**下游影响**（调用方 / 依赖 / 测试）。当前 collie 的 E2E Assessment 对测试做了类似要求，但没有对代码 blast radius 做等价要求。
...
- **执行**：dispatch `Skill("collie:review")` with `Mode=code`, `Target=<worktree diff>`, `Context="Plan: $ARCHIVE_PATH"`
...
- **Completion Promise 更新**：`collie:gated-workflow 返回成功` = 包含 `[collie-final-review]` PASS，无需 auto.md 额外校验
...
- CLAUDE.md 发布红线："必须 dogfood" — 调用 `/collie:auto` 真实验证
...
- 自动化 E2E 需要真实 `/collie:auto` 环境 + 真实 LLM 调用 + 时间成本高

### 结论

**`e2e_feasible: false`** — 理由：改动为 prose 指令变更。验证路径是 CLAUDE.md 已强制的 dogfood，即本 plan 合并后下一次 `/collie:auto` 调用自然会使用新流程。本轮执行时额外 dogfood（见下方 Execution Preamble）。
...
- **跨 session 并发**（`collie:queue` 任务队列 concurrency=1）→ 不受影响
...
**Goal:** (1) 在 plan 阶段强制 Impact Assessment 章节；(2) 将 collie:review Mode=code 从 auto.md Step ⑥ 前移到 gated-workflow TodoList 的 `[collie-final-review]` 节点，修复 worktree 清理时序 bug；(3) 吸收 Karpathy CLAUDE.md Principle 2/3，补充 Red line #13（Speculative scope）+ Q11（Surgical scope）到 rubric 体系。
...
2. 执行到 `[collie-final-review]` 时：dispatch `Skill("collie:review")` with `Mode=code`, `Target=<worktree 绝对路径>`, `Context="Plan: $ARCHIVE_PATH"`。PASS 才进入 `[finish]`；WARN / BLOCK 则就地 dispatch 修复 subagent，循环到 PASS。
...
- `[collie-final-review]`：整体 rubric 粒度，使用 `collie:review` Mode=code，关注**所有改动聚合后的 13 红线 + 11 问题 + ELEPHANT**
...
- 删除 `⑥ Skill(collie:review Mode=code) → final review`
...
③ PARALLEL: Agent(collie:plan-doc-reviewer) AND Skill(collie:review Mode=plan)
   → both must approve before ④
④ ExitPlanMode → TaskUpdate all planning tasks completed, close planning TaskList
⑤ collie:gated-workflow skill → complete implementation pipeline
   （内含 [collie-final-review] = Skill(collie:review Mode=code)，作为 [finish] 前的 pre-merge gate）
...
```
1. collie:review (Mode=code) returns `**Status:** PASS`
```

改为：

```
1. collie:gated-workflow returns successfully (含 [collie-final-review] 返回 `**Status:** PASS`)
```
...
```markdown
> **Completion** — 当 `collie:gated-workflow` 返回成功（内部 `[collie-final-review]` 返回 `**Status:** PASS`），output:
...
```
→ collie:review skill (Mode=code, Target=worktree diff)
...
```
→ collie:gated-workflow skill（内含 [collie-final-review] pre-merge gate） → <promise>Collie: SHIP IT</promise>
```
...
- **Pre-merge rubric gate**：`collie:review` Mode=code 作为 `[collie-final-review]` 节点嵌入 `gated-workflow` TodoList 中 `[doc-refresh]` 之后、`[finish]` 之前（Step 5.7）。worktree 清理前必须通过 rubric gate，auto.md 无独立 Step ⑥。
...
```
→ collie:review skill (Mode=code, Target=worktree diff, Context=plan doc)
→ PASS → <promise>Collie: SHIP IT</promise>
...
1. **单元测试**：`cd ~/git/collie && node --test tests/*.test.js` — 期望全绿（hook 逻辑未动）
2. **Plugin 结构验证**：`claude plugin validate ~/git/collie` — 期望 `✔ Validation passed`
3. **入口对应表审计**：`grep -n '/collie' README.md CLAUDE.md` 与 `ls commands/ skills/*/SKILL.md agents/*.md` 对照 — 期望无漂移
4. **文档同步手动核对**：
...
5. **Dogfood（本轮 Execution Preamble）**：
   - 执行 `[collie-final-review]` 时 dispatch `collie:review` Mode=code，观察 Target=worktree diff 能正确解析（因为本轮 [finish] 尚未执行）
