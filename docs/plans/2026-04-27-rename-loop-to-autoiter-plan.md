...
**问题 1**：`/collie-harness:loop` 与 Claude Code 内置全局 `loop` skill（Run a prompt or slash command on a recurring interval — `/loop 5m /foo`）重名。当前会话的 system reminder 中 `loop` 已被列入可调用的全局 skill 列表。用户在不带 `collie-harness:` 前缀键入 `/loop` 时，路由二义；命名冲突已成事实而非潜在风险。
...
**预期结果**：
1. 命令重命名为 `/collie-harness:autoiter`，全部 ~40 处引用同步迁移，无遗漏；hook regex + fixture test 原子改动
...
| 维度 | 旧值 | 新值 | 影响文件数 |
|------|------|------|----------|
| Slash 命令 | `/collie-harness:loop` | `/collie-harness:autoiter` | ~25 处文档引用 |
| 主 Skill 名 | `collie-harness:loop` | `collie-harness:autoiter` | SKILL.md frontmatter 1 处 + 引用 ~10 处 |
| 子 Skill 名 | `collie-harness:loop-prepare` | `collie-harness:autoiter-prepare` | SKILL.md frontmatter 1 处 + 引用 ~3 处 |
...
| 状态目录段 | `~/.collie-harness/loop/{project-id}/` | `~/.collie-harness/autoiter/{project-id}/` | `_state.js` 1 处常量 |
...
| Hook plan-executor | `collie-harness:loop` | `collie-harness:autoiter` | hook regex 1 处 + fixture test 2 处 |
...
| Queue schema enum | `command: /collie-harness:loop` | `command: /collie-harness:autoiter` | skills/queue/SKILL.md ~5 处 |
...
放在 `skills/autoiter/SKILL.md` 的 `# collie-harness:autoiter — Main Loop Orchestrator` 标题之下，**§3.5 Entry State Machine 之前**，作为所有 stage 的元约束。文本（最终落地以此为准）：
...
- **mv + 字面量替换**：`tests/loop.test.js` → `tests/autoiter.test.js`；fixture 字符串全量替换 `loop-stage0` → `autoiter-stage0`、`collie-harness:loop` → `collie-harness:autoiter`、`COLLIE_LOOP_*` → `COLLIE_AUTOITER_*`
...
- **e2e smoke**：scenario 5 改名 + grep 断言改为查找 `commands/autoiter.md` + `skills/autoiter/SKILL.md` 的 `name: collie-harness:autoiter`
...
| Skill name 字段 | scenario 5 grep `skills/autoiter/SKILL.md` 含 `name: collie-harness:autoiter` | `tests/e2e/smoke.sh` |
...
**Purpose**：实施开始前先全仓 grep 出所有 `COLLIE_LOOP_*` / `collie-harness:loop` / `loop-stage0` / `Collie: LOOP DONE` / `.worktrees/loop-` / `Loop Workflow` / `loop-iter-N-fixes` 的真实分布，与已列任务覆盖的文件做 set-diff，确保举一反三防御。
...
- [ ] **Step 1**: 全仓 grep（含 scripts/ 与所有 .sh / .md / .js / .json）
  ...
  grep -rln \
    -e 'COLLIE_LOOP_' \
    -e 'collie-harness:loop\b' \
    -e 'collie-harness:loop-prepare' \
    -e 'loop-stage0' \
    -e 'Collie: LOOP DONE' \
    -e '\.worktrees/loop-' \
    -e 'loop-iter-N-fixes' \
...
- [ ] **Step 4**: 输出 baseline 清单到 `~/.collie-harness/state/snappy-inventing-charm/baseline-loop-grep.txt`

  ```bash
  mkdir -p ~/.collie-harness/state/snappy-inventing-charm/
  ...
  grep -rln \
    ...
    | grep -v 'node_modules\|\.git/\|docs/plans/2026-04-24-loop-command-plan\.md' \
    | sort -u \
    > ~/.collie-harness/state/snappy-inventing-charm/baseline-loop-grep.txt
  wc -l ~/.collie-harness/state/snappy-inventing-charm/baseline-loop-grep.txt
  ```
...
- [ ] **Step 3**: Verify
  ...
  ```
  skills/autoiter/SKILL.md:name: collie-harness:autoiter
  skills/autoiter-prepare/SKILL.md:name: collie-harness:autoiter-prepare
  ```
...
- [ ] **Step 4**: Verify
  ...
  ```bash
  grep -n 'loop-stage0\|collie-harness:loop' hooks/post-writing-plans-reviewer.js
  ```
...
- [ ] **Step 1**: 全文替换
  ...
  sed -i.bak \
    ...
    -e 's|collie-harness:loop|collie-harness:autoiter|g' \
    -e "s|'loop/|'autoiter/|g" \
    -e 's|"loop/|"autoiter/|g' \
...
- [ ] **Step 2**: 检查环境变量字面量（`COLLIE_LOOP_*`）— 当前 fixture 不包含但保险起见 grep
  ...
  ```bash
  grep -n 'COLLIE_LOOP\|loop-stage0\|collie-harness:loop' tests/autoiter.test.js
  ```
...
- [ ] **Step 1**: commit
  ...
  - hooks/post-writing-plans-reviewer.js: regex 'loop-stage0' / 'collie-harness:loop' 同步改 'autoiter-stage0' / 'collie-harness:autoiter'
...
- [ ] **Step 1**: 找到 `# collie-harness:autoiter — Main Loop Orchestrator` 标题（约 line 11）的下一行 `## §3.5 Entry State Machine` 之前的位置（约 line 94 之前），插入 Section 0 整段。
...
- [ ] **Step 2**: 同步全文 `collie-harness:loop` → `collie-harness:autoiter`

  ```bash
  sed -i.bak 's|collie-harness:loop|collie-harness:autoiter|g' skills/autoiter/references/fix-plan-template.md
...
- [ ] **Step 1**: 批量替换
  ...
  for f in \
    ...
    sed -i.bak \
      -e 's|collie-harness:loop-prepare|collie-harness:autoiter-prepare|g' \
      -e 's|collie-harness:loop|collie-harness:autoiter|g' \
...
- [ ] **Step 2**: Verify 无残留
  ...
  ```bash
  grep -rn 'collie-harness:loop\|COLLIE_LOOP' skills/autoiter*/references/
  ```

  Expected：无输出。

- [ ] **Step 3**: 同步更新子 SKILL（`skills/autoiter-prepare/SKILL.md`）的 description 字段（若包含 `collie-harness:loop` 引用）

  ```bash
  grep -n 'collie-harness:loop' skills/autoiter-prepare/SKILL.md
  sed -i.bak \
    -e 's|collie-harness:loop-prepare|collie-harness:autoiter-prepare|g' \
    -e 's|collie-harness:loop|collie-harness:autoiter|g' \
...
- [ ] **Step 1**: 替换 skill 名 + sentinel + 内嵌引用
  ...
  ```bash
  sed -i.bak \
    -e "s|Skill('collie-harness:loop')|Skill('collie-harness:autoiter')|g" \
    -e 's|collie-harness:loop|collie-harness:autoiter|g' \
...
- [ ] **Step 5**: Verify 无残留
  ...
  ```bash
  grep -rn 'collie-harness:loop\|COLLIE_LOOP\|Collie: LOOP DONE\|\.worktrees/loop-' \
...
- [ ] **Step 1**: 替换字面量
  ...
  ```bash
  sed -i.bak 's|/collie-harness:loop|/collie-harness:autoiter|g' skills/queue/SKILL.md
...
- [ ] **Step 2**: Verify
  ...
  ```bash
  grep -n 'collie-harness:loop\b' skills/queue/SKILL.md
  ```
...
- [ ] **Step 2**: 跑残留 grep（防止有遗漏字面量）
  ...
  ```bash
  grep -rn 'collie-harness:loop\b\|COLLIE_LOOP_\|Collie: LOOP DONE\|loop-stage0' \
...
- [ ] **Step 1**: 章节标题 + 散文术语替换
  ...
  ```bash
  sed -i.bak \
    -e 's|## Loop Workflow（`/collie-harness:loop`|## Autoiter Workflow（`/collie-harness:autoiter`|g' \
    -e 's|/collie-harness:loop|/collie-harness:autoiter|g' \
    -e 's|`collie-harness:loop-prepare`|`collie-harness:autoiter-prepare`|g' \
    -e 's|`collie-harness:loop`|`collie-harness:autoiter`|g' \
    -e 's|loop-stage0|autoiter-stage0|g' \
...
- [ ] **Step 4**: Verify
  ...
  ```bash
  grep -n 'collie-harness:loop\b\|COLLIE_LOOP_\|Loop Workflow\|Collie: LOOP DONE' CLAUDE.md
  ```
...
- [ ] **Step 2**: Verify
  ...
  ```bash
  grep -n 'collie-harness:loop\b\|COLLIE_LOOP\|`/loop`\|Loop 循环\|loop/{project-id}\|loop\.md' README.md
  ```
...
- [ ] **Step 3**: 全维度检查 `marketplace.json`（version + 命令引用 + 描述残留）
  ...
  grep -nE '0\.2\.4|"version"|/collie-harness:loop\b|`/loop`|loop command|collie-harness:loop\b' .claude-plugin/marketplace.json || echo "no hits"
  ```
  ...
  **b) 命令名引用（如 `/collie-harness:loop` / `\`/loop\``）**：必须替换为 autoiter 等价物
...
- [ ] **Step 4**: Verify 修复完整性
  ...
  # 必须无 collie-harness:loop（不带 -prepare 后缀）残留
  grep -nE 'collie-harness:loop\b' .claude-plugin/marketplace.json && echo "FAIL: stale command ref" || echo "OK: no stale command ref"
  ```
...
- [ ] **Step 1**: plugin validate
  ...
  ```bash
  claude plugin validate ~/git/collie-harness 2>&1 | tail -5
  ```
...
- [ ] **Step 4**: 残留全仓 grep（最终一道防线，字面量取自 §Section 1 重命名映射表全集）
  ...
  **Diff vs B0-T0 baseline**：
  ...
  comm -12 /tmp/autoiter-final-grep.txt ~/.collie-harness/state/snappy-inventing-charm/baseline-loop-grep.txt
  ```
...
- [ ] **Step 5**: 实施 agent 注意事项（gated-workflow 内置 gate 显式提醒）
  ...
  本 plan 元数据 `<!-- plan-executor: collie-harness:gated-workflow -->` 已声明 `gated-workflow` 是执行 skill。`gated-workflow` 自带 `[collie-final-review]` 节点（Step 5.7，在 `[doc-refresh]` 之后、`[finish]` 之前），调用 `Skill('collie-harness:review')` Mode=code 做 pre-merge rubric gate。
