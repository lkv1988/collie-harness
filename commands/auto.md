---
description: "Launch Collie-style fully automated feature development loop (brainstorm → plan → review → flow → rubric review)"
...
1. collie:flow returns successfully (含 [collie-final-review] 返回 `**Status:** PASS`)
...
The hook will block ExitPlanMode anyway. Both `collie:plan-doc-reviewer` AND `collie:review` (Mode=plan) must return approval. There are no shortcuts.
...
**"flow 跑完直接 SHIP，rubric review 不必要"**
flow 内部已含 `[collie-final-review]` pre-merge gate。试图省略 = Step 5.7 GATE 违规 = red line。
...
⑥ collie:flow skill → complete implementation pipeline
   （内含 [collie-final-review] = Skill(collie:review Mode=code)，作为 [finish] 前的 pre-merge gate）
⑦ flow 返回成功 → output completion signal
```
...
> - [plan-review] Structural plan review (collie:plan-doc-reviewer)
> - [collie-review] Collie rubric review (collie:review Mode=plan)
...
>     - Record this path as `$PLAN_PATH`. These three lines are the only mechanism that survives the "clear context and execute" boundary — flow depends on them.
...
>       |------|-------|------------|-----------|
>       ```
>       `Key files` lists files each task creates or modifies. The flow plan-reader subagent depends on this table.
...
>     - **Skip writing-plans Plan Review Loop**: writing-plans' built-in plan-document-reviewer per-chunk review is skipped in collie. collie Step ③ has stricter dual-reviewer review — do not run both.
>     - **Skip writing-plans Execution Handoff**: writing-plans' "Ready to execute?" prompt and skill recommendation are skipped in collie. After the plan is written, return directly to auto.md Step ③ dual review.
...
>   a) `Agent(subagent_type="collie:plan-doc-reviewer", model="opus")` — structural plan validation
>   b) `Skill("collie:review")` with `Mode=plan`, `Target=$PLAN_PATH` — Collie-style rubric review
...
> </HARD-GATE>
>
> **ExitPlanMode** — after returning from planmode, use TaskUpdate to mark all planning tasks ([research], [plan-review], [collie-review], [exit]) as completed. brainstorming 的 9 条子任务由 brainstorming skill 自身负责标记完成，无需我们管理。This closes the planning TaskList before flow appends the implementation tasks.
>
> **Implementation** — call `collie:flow` skill.
>
> **Completion** — 当 `collie:flow` 返回成功（内部 `[collie-final-review]` 返回 `**Status:** PASS`），output:
> `<promise>Collie: SHIP IT</promise>`
>
> 若 flow 内部出现无法自愈的 WARN / BLOCK（连续 3 轮修复失败），通过 `scripts/escalate.sh` 升级。
